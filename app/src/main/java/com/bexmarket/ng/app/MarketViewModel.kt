    ...
    fun fetchProducts() {
        viewModelScope.launch {
            ...
            try {
                ...
                Log.d("MarketViewModel", "Successfully fetched ${products.size} products")
                allItems = products
                _uiState.value = MarketUiState.Success(products)
            } catch (e: retrofit2.HttpException) {
                val errorBody = e.response()?.errorBody()?.string()
                Log.e("MarketViewModel", "HTTP Error ${e.code()}: $errorBody")
                loadSampleData("HTTP ${e.code()}")
            } catch (e: Exception) {
                Log.e("MarketViewModel", "Network/Parsing Error: ${e.message}", e)
                loadSampleData(e.message ?: "Unknown error")
            }
        }
    }

    private fun loadSampleData(errorMessage: String) {
